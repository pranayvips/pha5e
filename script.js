      ...
      let done = false;
        container.addEventListener("mouseenter", (details) => {
            ...
            heroThumbnailsDiv.forEach((container, idx) => {
                if (idx != index) {
                    container.children[0].classList.add("non-active");
                }else if(window.innerWidth > 1050){
                    container.children[0].children[1].style.display= "block";
                }
            });
            heroText.classList.add("hero-text-hover");
            done = true;
        });
        ...
        container.addEventListener("mouseleave", (details) => {
            ...
            heroThumbnailsDiv.forEach((container, idx) => {
                if (idx != index) {
                    container.children[0].classList.remove("non-active");
                }else if(window.innerWidth > 1050){
                    container.children[0].children[1].style.display= "none";
                }
        });
        heroText.classList.remove("hero-text-hover")
      });
    });    
}, splashTimeout);
